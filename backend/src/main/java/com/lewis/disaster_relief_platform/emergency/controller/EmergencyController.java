...
public class EmergencyController {
    ...
    @Operation(summary = "Get current volunteer assignments", description = "Fetches emergencies assigned to the logged-in volunteer.")
    public ResponseEntity<ApiResponse<PageResponse<EmergencyResponse>>> getMyAssignedEmergencies(
        ...
        Sort.Direction direction = sortDirection.equalsIgnoreCase("ASC") ? Sort.Direction.ASC : Sort.Direction.DESC;
        Pageable pageable = PageRequest.of(page, size, Sort.by(direction, sortBy));
        Page<EmergencyResponse> emergencies = emergencyService.getAssignedToCurrentVolunteer(pageable);
        // This route exists so volunteers do not need admin-only access just to see work assigned to their MongoDB user id.
        return ResponseEntity.ok(ApiResponse.success(PageResponse.of(emergencies)));
    }
     ...
     */
    @GetMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN') or hasRole('COORDINATOR') or hasRole('VOLUNTEER')")
    ...
    @PreAuthorize("hasRole('ADMIN') or hasRole('COORDINATOR') or hasRole('VOLUNTEER')")
    @Operation(summary = "Update emergency progress", description = "Persists task progress, completion notes, and completion actor details.")
    public ResponseEntity<ApiResponse<EmergencyResponse>> updateEmergencyProgress(
            @PathVariable String id,
            @Valid @RequestBody EmergencyStatusUpdateRequest request) {
        // The frontend uses this endpoint for Mark Active and Mark Completed so task progress is stored in MongoDB, not browser storage.
        EmergencyResponse response = emergencyService.updateEmergencyProgress(id, request);
        return ResponseEntity.ok(ApiResponse.success(response, "Emergency progress updated successfully."));
    }

    /**
     * Assign volunteer to emergency
     * PATCH /api/v1/emergencies/{emergencyId}/assign/{volunteerId}
     */
    @PatchMapping({"/{emergencyId}/assign/{volunteerId}", "/{emergencyId}/{volunteerId}"})
    @PreAuthorize("hasRole('ADMIN') or hasRole('COORDINATOR')")
    ...
    public ResponseEntity<ApiResponse<EmergencyResponse>> assignVolunteer(@PathVariable String emergencyId, @PathVariable String volunteerId) {
        // Keep the legacy two-segment mapping temporarily, but prefer /assign so assignment cannot be confused with a generic id lookup.
