...
public interface EmergencyRepository extends MongoRepository<Emergency, String> {
    ...
    // Volunteers should only load requests assigned to their MongoDB user id.
    ...
    @Query("{'$or': [{'title': {$regex: ?0, $options: 'i'}}, {'description': {$regex: ?0, $options: 'i'}}, {'trackingCode': {$regex: ?0, $options: 'i'}}, {'location.address': {$regex: ?0, $options: 'i'}}, {'location.city': {$regex: ?0, $options: 'i'}}, {'location.state': {$regex: ?0, $options: 'i'}}, {'location.zipCode': {$regex: ?0, $options: 'i'}}]}")
    ...
    Page<Emergency> findByLocationZipCode(String zipCode, Pageable pageable);
