...
const extractAlertAreas = (areaDesc = '') => {
  return `${areaDesc}`
    .split(';')
    .map((area) => area.trim())
    .filter(Boolean);
...
const buildFocusAreaSummary = (alerts) => {
  const counts = alerts.reduce((accumulator, alert) => {
    extractAlertAreas(alert.areaDesc).forEach((area) => {
...
const formatAlertWindow = (alert) => {
  const start = alert.effective || alert.sent;
  const end = alert.ends;
  ...
  if (start && end) {
    return `${formatTimestamp(start, { hour: 'numeric', minute: '2-digit' })} to ${formatTimestamp(end, {
      month: 'short',
      day: 'numeric',
      hour: 'numeric',
      minute: '2-digit',
  ...
  if (start) {
    return `Started ${formatTimestamp(start, { hour: 'numeric', minute: '2-digit' })}`;
  }
...
const LiveActivityPage = () => {
  ...
  const focusAreas = useMemo(() => buildFocusAreaSummary(alerts), [alerts]);
            ...
            {focusAreas.length > 0 && (
              <div className="live-focus-areas">
                <div className="live-focus-header">
                  ...
                  <span>Most repeated impacted zones across current severe NOAA alerts.</span>
                </div>
                <div className="live-focus-tags">
                  {focusAreas.map((area) => (
                    <span key={area.label} className="live-focus-tag" title={area.label}>
                      <span className="live-focus-tag-label">{area.label}</span>
                      <span className="live-focus-tag-count">{area.count}</span>
                    </span>
                  ))}
                </div>
              </div>
              ...
              <div className="live-critical-grid">
                {alerts.slice(0, 8).map((alert) => (
                  <article key={alert.id} className="live-critical-card">
                    ...
                    <p className="live-critical-area" title={alert.areaDesc}>
                      <MapPin size={13} />
                      <span>{alert.areaDesc}</span>
                    </p>
                    <p className="live-critical-headline">{alert.headline}</p>
                    <div className="live-critical-times">
                      <span><Clock size={12} /> {formatAlertWindow(alert)}</span>
                    </div>
                    <div className="live-critical-meta-row">
                      <span className="live-critical-meta-pill">Status: {alert.status}</span>
                      <span className="live-critical-meta-pill">Urgency: {alert.urgency}</span>
                      <span className="live-critical-meta-pill">Certainty: {alert.certainty}</span>
                    </div>
