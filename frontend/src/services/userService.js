...
function mapUser(u) {
  ...
  const role = typeof rawRole === 'string'
    ? (
      rawRole.replace(/^ROLE_/, '').toUpperCase() === 'COORDINATOR'
        ? 'organization_staff'
...
function mapVolunteer(volunteer) {
  const availability = String(volunteer.availabilityStatus ?? 'UNAVAILABLE').toUpperCase();
  return {
    id: volunteer.id,
    ...
    role: 'volunteer',
    status: availability === 'UNAVAILABLE' ? 'inactive' : 'active',
    availabilityStatus: availability.toLowerCase(),
    verified: Boolean(volunteer.verified),
    completedAssignments: volunteer.completedAssignments ?? 0,
    activeAssignments: volunteer.activeAssignments ?? 0,
    skills: Array.isArray(volunteer.skills) ? volunteer.skills : [],
    createdAt: volunteer.registeredAt ?? null,
    lastLogin: volunteer.lastActiveAt ?? null,
  };
}
...
export async function createUser(data, token) {
  try {
    ...
    if (data.role && data.role !== 'volunteer') {
      return {
        success: false,
        message: 'The backend register endpoint currently creates volunteer accounts only.',
      };
    }
...
export async function getVolunteers(token, filters = {}) {
  try {
    const page = filters.page ?? 0;
    const size = filters.size ?? 100;
    const data = await apiFetch(`/volunteers?page=${page}&size=${size}&sortBy=registeredAt&sortDirection=DESC`, token);
    let volunteers = (data?.content ?? data ?? []).map(mapVolunteer);
    ...
    if (filters.status && filters.status !== 'all') {
      volunteers = volunteers.filter((volunteer) => volunteer.status === filters.status);
    }

    return { success: true, volunteers, total: data?.totalElements ?? volunteers.length };
  } catch (err) {
    return { success: false, volunteers: [], message: err.message };
  }
}
...
export async function changeUserRole(userId, newRole, token) {
  return {
    success: false,
    message: 'Role changes are not available because the current backend does not expose a role update endpoint.',
  };
}
